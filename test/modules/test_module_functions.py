...
# Removed TestModuleFunctions class wrapper, tests are now functions
...
def test_get_embedding() -> None:
    ...
    assert isinstance(embedded_tensor, tf.Tensor)
    assert embedded_tensor.shape == (batch_size, 1, 1, embedding_dims)
...
def test_build_encoder() -> None:
    ...
    assert isinstance(inherited_layers, list)
    assert len(inherited_layers) == len(layer_kernels)

    expected_height = height // (2**len(layer_kernels))
    expected_width = width // (2**len(layer_kernels))
    expected_channels = layer_kernels[-1]

    assert list(output_layer.shape) == [None, expected_height, expected_width, expected_channels]
    for i, inherited_layer in enumerate(inherited_layers):
        assert isinstance(inherited_layer, tf.keras.KerasTensor)
        assert list(inherited_layer.shape) == [None, height // (2**i), width // (2**i), layer_kernels[i]]

def test_build_decoder() -> None:
    start_height: int = 8
    start_width: int = 8
...
def test_build_decoder_no_inherited_layers() -> None:
    start_height: int = 8
    start_width: int = 8
    start_channels: int = 64
    ...
    assert isinstance(output_layer_no_skip, tf.keras.KerasTensor)
    expected_height = start_height * (2**len(decoder_layer_kernels))
    expected_width = start_width * (2**len(decoder_layer_kernels))
    expected_channels = decoder_layer_kernels[0]
    assert list(output_layer_no_skip.shape) == [None, expected_height, expected_width, expected_channels]
