...
class TestBlurryGenerator(unittest.TestCase):
    ...
    def test_instantiation(self) -> None:
        ...
        self.assertIsInstance(blurry_gen, BlurryGenerator)
        self.assertEqual(blurry_gen.generator, mock_inner_generator)

    @patch('src.generators.blurry.ndimage.gaussian_filter')
    def test_blur_method(self, mock_gaussian_filter: MagicMock) -> None:
        ...
        self.assertIsInstance(blurred_array, np.ndarray)
        self.assertEqual(blurred_array.shape, input_array.shape)
        ...
        self.assertAlmostEqual(args[1], sigma) # args[0] is the array
        self.assertEqual(kwargs.get('axes'), (0,1))
        # Check if output is different (assuming filter changes it)
        self.assertFalse(np.array_equal(blurred_array, input_array))


    @patch.object(BlurryGenerator, 'blur')
    def test_next_method(self, mock_blur_method: MagicMock) -> None:
        batch_size: int = 2
        ...
        self.assertIsInstance(processed_inputs, np.ndarray)
        self.assertIsInstance(processed_outputs, np.ndarray)

        self.assertEqual(processed_inputs.shape, (batch_size, height, width, channels))
        # Output has an extra channel for sigma
        self.assertEqual(processed_outputs.shape, (batch_size, height, width, channels + 1))

        # Check blur method was called for inputs and outputs
        # It's called per item, so batch_size * 2 times
        self.assertEqual(mock_blur_method.call_count, batch_size * 2)

        # Check that the sigma channel in output is within [0, 1]
        sigma_channel: np.ndarray = processed_outputs[..., -1]
        self.assertTrue(np.all(sigma_channel >= 0))
        self.assertTrue(np.all(sigma_channel <= 1))
        self.assertEqual(sigma_channel.shape, (batch_size, height, width))
