...
# Removed TestBatchGenerator class wrapper, tests are now functions
...
def test_instantiation() -> None:
    ...
    assert generator.shape == shape
    assert np.array_equal(generator.filenames, np.array(filenames))

@patch('src.generators.base_generators.ThreadPool')
def test_iteration_and_batch_processing_success(mock_thread_pool_class: MagicMock) -> None:
    ...
    assert inputs.shape == (batch_size, height, width, channels)
    ...
    assert generator.reader.call_count == batch_size


@patch('src.generators.base_generators.ThreadPool')
def test_iteration_and_batch_processing_reader_failure(mock_thread_pool_class: MagicMock) -> None:
