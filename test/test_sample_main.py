...
def test_imread_rgb_basic_resize_and_normalize() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert input_data.shape == (32, 32, 3)
        assert input_data.dtype == np.float32
        assert np.all((input_data >= 0) & (input_data <= 1))

def test_imread_rgba_to_rgb_conversion() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert input_data.shape == (48, 48, 3)
        assert input_data.dtype == np.float32
        assert np.all((input_data >= 0) & (input_data <= 1))

def test_imread_grayscale_to_3_channel_conversion() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_imread_grayscale_to_1_channel_no_conversion() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert input_data.shape == (32, 32, 1)
        assert input_data.dtype == np.float32
        assert np.all((input_data >= 0) & (input_data <= 1))

def test_imread_no_resize_if_dimensions_match() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert input_data.shape == (40, 40, 3)
        assert input_data.dtype == np.float32
        assert np.all((input_data >= 0) & (input_data <= 1))

def test_imread_interpolation_flags() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        original_size = (10, 10)
        target_size = (20, 20)
        ...
        for name, flag in interpolations.items():
            ...
            assert input_data.shape == target_shape, f"Shape mismatch for {name} interpolation"
            assert input_data.dtype == np.float32, f"Dtype mismatch for {name} interpolation"
        ...
        assert not np.array_equal(results["nearest"], results["linear"]), "Nearest and Linear should differ"
        assert not np.array_equal(results["linear"], results["cubic"]), "Linear and Cubic should differ"
        assert not np.array_equal(results["nearest"], results["cubic"]), "Nearest and Cubic should differ"

def test_input_data_caching() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert "_lazy_input_data" in sample.__dict__, "LazyProperty cache attribute not found."
...
def test_imread_non_existent_file() -> None:
